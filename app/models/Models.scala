...
case class Issue(id: Long, issueName: String, milestone: Milestone)

object Issue {
  implicit val issueReads: Reads[Issue] = (
    (JsPath \ "id").read[Long] and
      (JsPath \ "title").read[String] and
      (JsPath \ "milestone").read[Milestone].orElse(Reads.pure(new Milestone(0, "(No milestone)")))) (Issue.apply _)
}
